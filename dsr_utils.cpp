    ...
    foreach (const QJsonValue & linkValue, linksArray)
    {
        ...
        for(QVariantMap::const_iterator iter = linkAttributesMap.begin(); iter != linkAttributesMap.end(); ++iter)
        {
            ...
            switch (attr_type) 
            {
                case 0:
                {
                    G->add_attrib(attrs, attr_key, attr_value.toString().toStdString());
                    break;
                }
                case 1:
                {
                ...
                case 2:
                {
                    G->add_attrib(attrs, attr_key, attr_value.toString().replace(",", ".").toFloat());
                    break;
                }
                case 3: 
                {
