...
namespace DSR {
    ...
    FastDDSTransport::FastDDSTransport(std::weak_ptr<Transport> _transport) 
        : tp(5), tp_delta_attr(2) 
    {
                    ...
                    if (m_info.instance_state == eprosima::fastdds::dds::ALIVE_INSTANCE_STATE) {
                        if (sample.id() != graph->get_agent_id()) {
                            if (sample.id() != static_cast<uint32_t>(-1)) {
                                std::cout << " Received Full Graph from " << m_info.sample_identity.writer_guid().entityId.value
                                        << " whith "
                    ...
                    if (m_info.instance_state == eprosima::fastdds::dds::ALIVE_INSTANCE_STATE) {
                        {
                            ...
                            if (auto [it, ok] = tr->participant_set.emplace(sample.from(), true);
                                it->second and !ok)
                            {
                                if (it->second) {
                                    lck.unlock();
    ...
    auto FastDDSTransport::start_subs(Graph *graph, bool show) -> void 
    {
        ...
        auto delta_node_attrs_thread = std::async(std::launch::async, &FastDDSTransport::start_node_attrs_subscription, this, graph, show);
        auto delta_edge_attrs_thread = std::async(std::launch::async, &FastDDSTransport::start_edge_attrs_subscription, this, graph, show);
    ...
    auto FastDDSTransport::start_node_subscription(Graph *graph, bool show) -> void
    {
        ...
        auto lambda_general_topic = [&, name = name, showReceived = show]
        (DDSReader *reader, Graph* graph)
        {
            try {
                while (true)
                {
                    ...
                    if (reader->take_next_sample(&sample, &m_info) == ReturnCode_t::RETCODE_OK) {
                        if (m_info.instance_state == eprosima::fastdds::dds::ALIVE_INSTANCE_STATE) {
                            if (sample.agent_id() != graph->get_agent_id()) {
                                if (showReceived) {
                                    std::cout << name << " Received:" << std::to_string(sample.id()).c_str() << " node from: "
                                            << m_info.sample_identity.writer_guid().entityId.value << "\n";
                                }
    ...
    {
        auto name = __FUNCTION__;
        auto lambda_general_topic = [this, name = name, showReceived = show]
        (DDSReader *reader, Graph *graph)
        {
            try {
                while (true)
                {
                    ...
                    if (reader->take_next_sample(&samples, &m_info) == ReturnCode_t::RETCODE_OK) {
                        if (m_info.instance_state == eprosima::fastdds::dds::ALIVE_INSTANCE_STATE) {
                            if (showReceived) {
                                std::cout << name << " Received:" << samples.vec().size() << " node attrs from: "
                                        << m_info.sample_identity.writer_guid().entityId.value << "\n";
                            }
                            if (!samples.vec().empty() and samples.vec().at(0).agent_id() != graph->get_agent_id()) {
                                tp_delta_attr.spawn_task([this, graph, samples = std::move(samples)]() mutable {
                                    ...
                                    emit graph->update_node_attr_signal(id, sig, SignalInfo{samples.vec().at(0).agent_id()});
                                    emit graph->update_node_signal(id, type, SignalInfo{samples.vec().at(0).agent_id()});
                                });
                            }
                        }
                    } else {
            ...
            try {
                while (true)
                {
                    ...
                    if (reader->take_next_sample(&samples, &m_info) == ReturnCode_t::RETCODE_OK) {
                        if (m_info.instance_state == eprosima::fastdds::dds::ALIVE_INSTANCE_STATE) {
                            if (showReceived) {
                                std::cout << name << " Received:" << samples.vec().size() << " edge attr from: "
                                        << m_info.sample_identity.writer_guid().entityId.value << "\n";
                            }
                            if (!samples.vec().empty() and samples.vec().at(0).agent_id() != graph->get_agent_id())
                            {
