    ...
    if (inserted)
    {
        if (!config.copy)
        {
            if (delta.has_value())
            {
                ...
                {
                    emit update_edge_signal(node.id(), k.first, k.second,  SignalInfo{static_cast<uint32_t>(config.agent_id)});
                }
            }
        }
        return node.id();
...
requires (std::is_same_v<std::remove_cvref_t<Ed>, DSR::Edge>)
{
...

    {
        ...
        if (graph->nodes.contains(from) && graph->nodes.contains(to)) {
            std::tie(result, delta_edge, delta_attrs) = graph->insert_or_assign_edge_(user_edge_to_crdt(std::forward<Ed>(attrs)), from, to);
        } else {
    ...
    if (result) {
        if (!config.copy) {
            ...
            if (delta_attrs.has_value()) { //Update
                ...
                emit update_edge_attr_signal(attrs.from(), attrs.to(), attrs.type(), atts_names, SignalInfo{ config.agent_id });
...
void DSRGraph::read_from_json_file(const std::string &file)
{
    utils->read_from_json_file(
        ...
        [&](const Node &node) -> std::optional<uint64_t>
        {
            ...
            {
                std::unique_lock<std::shared_mutex> lock(graph->_mutex_data);
                if (auto t1 = graph->id_map.find(node.id()) == graph->id_map.end(), t2 = graph->name_map.find(node.name()) == graph->name_map.end();
                    t1 and t2)
                {
