...
CRDTSyncEngine::CRDTSyncEngine(DSRGraph& host)
    : host_(host)
{
}

CRDTSyncEngine::CRDTSyncEngine(DSRGraph& host, const CRDTSyncEngine& other)
    : host_(host),
      nodes_(other.nodes_),
...
void CRDTSyncEngine::apply_remote_node_attr_batch(NodeAttrDeltaBatchMessage&& batch)
{
    ...
    for (auto&& item : payload->vec) {
        if (host_.ignored_attributes.contains(item.attr_name)) {
            continue;
        }
    ...
    if (!changed_attributes.empty()) {
        std::string type;
        {
            std::shared_lock<std::shared_mutex> lock(host_._mutex);
            if (const auto* node = get_node_ptr(id); node != nullptr) {
                type = node->type();
            }
        }
        host_.emitter.update_node_attr_signal(id, changed_attributes, SignalInfo{sample_agent_id});
        host_.emitter.update_node_signal(id, type, SignalInfo{sample_agent_id});
    }
}
...
void CRDTSyncEngine::apply_remote_edge_attr_batch(EdgeAttrDeltaBatchMessage&& batch)
{
    ...
    if (!changed_attributes.empty()) {
        host_.emitter.update_edge_attr_signal(from, to, type, changed_attributes, SignalInfo{sample_agent_id});
        host_.emitter.update_edge_signal(from, to, type, SignalInfo{sample_agent_id});
    }
}
...
FullGraphMessage CRDTSyncEngine::export_full_graph() const
{
    std::shared_lock<std::shared_mutex> lock(host_._mutex);
    OrMap map;
    map.id = static_cast<int32_t>(host_.agent_id);
    map.protocol_version = DSR_PROTOCOL_VERSION;
    map.sync_mode = sync_mode_wire_value(host_.sync_mode);
...
std::tuple<bool, std::optional<MvregNodeMsg>> CRDTSyncEngine::insert_node_raw(CRDTNode&& node)
{
    ...
    if (!host_.deleted.contains(node.id()))
    {
        ...
        uint64_t id = node.id();
        host_.update_maps_node_insert(id, node);
        auto delta = nodes_[id].write(std::move(node));
        return {true, CRDTNode_to_Msg(host_.agent_id, id, std::move(delta))};
    }
    return {false, {}};
}
            ...
            for (auto& [k, att] : node.attrs()) {
                ...
                if (attr_reg.empty() || att.read_reg() != attr_reg.read_reg()) {
                    ...
                    atts_deltas.vec.emplace_back(
                        CRDTNodeAttr_to_Msg(host_.agent_id, node.id(), node.id(), k, std::move(delta)));
                }
            }
            auto it_a = iter.begin();
            while (it_a != iter.end()) {
                const std::string& k = it_a->first;
                if (host_.ignored_attributes.contains(k)) {
...
CRDTSyncEngine::delete_node_raw(uint64_t id, const CRDTNode& node)
{
    ...
    MvregNodeMsg delta_remove = CRDTNode_to_Msg(host_.agent_id, id, std::move(delta));
    {
        decltype(host_.to_edges)::mapped_type incoming;
        {
            std::shared_lock<std::shared_mutex> lck_cache(host_._mutex_cache_maps);
            if (host_.to_edges.contains(id))
                incoming = host_.to_edges.at(id);
        }
        for (const auto& [from, type] : incoming)
        {
            ...
            delta_vec.emplace_back(CRDTEdge_to_Msg(host_.agent_id, from, id, type, std::move(delta_fano)));
            visited_node.fano().erase({id, type});
            host_.update_maps_edge_delete(from, id, type);
        }
    }

    host_.update_maps_node_delete(id, node);
...
std::optional<MvregEdgeMsg> CRDTSyncEngine::delete_edge_raw(uint64_t from, uint64_t to, const std::string& key)
{
    if (nodes_.contains(from)) {
        ...
        if (node.fano().contains({to, key})) {
            ...
            return CRDTEdge_to_Msg(host_.agent_id, from, to, key, std::move(delta));
        }
    }
    return {};
}
...
CRDTSyncEngine::insert_or_assign_edge_raw(CRDTEdge&& attrs, uint64_t from, uint64_t to)
{
    ...
    if (nodes_.contains(from))
    {
        ...
        if (fano_it != node.fano().end())
        {
            ...
            for (auto& [k, att] : attrs.attrs()) {
                ...
                if (attr_reg.empty() || att.read_reg() != attr_reg.read_reg()) {
                    auto delta = attr_reg.write(std::move(att.read_reg()));
                    atts_deltas.vec.emplace_back(
                        CRDTEdgeAttr_to_Msg(host_.agent_id, from, from, to, attrs.type(), k, std::move(delta)));
                }
            }
            ...
            while (it != iter_edge.end()) {
                if (!attrs.attrs().contains(it->first)) {
                    ...
                    atts_deltas.vec.emplace_back(
                        CRDTEdgeAttr_to_Msg(host_.agent_id, from, from, to, attrs.type(), std::move(att), std::move(delta)));
                } else {
                    ++it;
                }
            }
        ...
        } else
        {
            ...
            return {true, CRDTEdge_to_Msg(host_.agent_id, from, to, std::move(att_type), std::move(delta)), {}};
        }
    }
    return {false, {}, {}};
}
...
void CRDTSyncEngine::join_delta_node(MvregNodeMsg&& mvreg)
{
    ...
    const auto log_level = host_.log_level;

    std::optional<CRDTNode> maybe_deleted_node = {};
    try {
        if (!protocol_version_matches(host_.log_level, "DSR_NODE", mvreg.protocol_version)) {
            return;
        }
...

        {
            ...
            std::unique_lock<std::shared_mutex> lock(host_._mutex);
            if (!host_.deleted.contains(id)) {
                ...
                if (nodes_.at(id).empty() || d_empty) {
                    if (maybe_deleted_node.has_value()) {
                        cache_map_to_edges = host_.to_edges[id];
                    }
                    host_.update_maps_node_delete(id, maybe_deleted_node);
                    delete_unprocessed_deltas();
                } else {
                    const auto& reg = nodes_.at(id).read_reg();
                    current_type = reg.type();
                    host_.update_maps_node_insert(id, reg);
        ...
        if (joined) {
            ...
            if (signal) {
                host_.emitter.update_node_signal(id, current_type, SignalInfo{ mvreg.agent_id });
                if (const auto* current = get_node_ptr(id); current != nullptr) {
                    for (const auto &[k, v] : current->fano()) {
                        host_.emitter.update_edge_signal(id, k.first, k.second, SignalInfo{ mvreg.agent_id });
                    }
                }

                for (const auto &[k, v]: map_new_to_edges)
                {
                    host_.emitter.update_edge_signal(k, id, v, SignalInfo{ mvreg.agent_id });
                }
            } else {
                host_.emitter.del_node_signal(id, SignalInfo{ mvreg.agent_id });
                if (maybe_deleted_node.has_value()) {
                    Node tmp_node(*maybe_deleted_node);
                    host_.emitter.deleted_node_signal(tmp_node, SignalInfo{ mvreg.agent_id });
                    for (const auto &node: maybe_deleted_node->fano()) {
                        host_.emitter.del_edge_signal(node.second.read_reg().from(), node.second.read_reg().to(),
                                             node.second.read_reg().type(), SignalInfo{ mvreg.agent_id });
                        Edge tmp_edge(node.second.read_reg());
                        host_.emitter.deleted_edge_signal(tmp_edge, SignalInfo{ mvreg.agent_id });
                    }
                }

                if (cache_map_to_edges.has_value()) {
                    for (const auto &[from, type] : cache_map_to_edges.value()) {
                        host_.emitter.del_edge_signal(from, id, type, SignalInfo{ mvreg.agent_id });
                    }
                }
            }
        }
...
void CRDTSyncEngine::join_delta_edge(MvregEdgeMsg&& mvreg)
{
    ...
    const auto log_level = host_.log_level;
    try {
        if (!protocol_version_matches(host_.log_level, "DSR_EDGE", mvreg.protocol_version)) {
            return;
        }
        bool signal = false, joined = false;
...

        {
            ...
            bool dfrom{host_.deleted.contains(from)}, dto{host_.deleted.contains(to)};
        ...
        if (joined) {
            ...
            if (signal) {
                host_.emitter.update_edge_signal(from, to, type, SignalInfo{ mvreg.agent_id });
            } else {
                host_.emitter.del_edge_signal(from, to, type, SignalInfo{ mvreg.agent_id });
                if (deleted_edge.has_value()) {
                    host_.emitter.deleted_edge_signal(*deleted_edge, SignalInfo{ mvreg.agent_id });
                }
            }
        }
    } catch (const std::exception &ex) { std::cerr << ex.what() << std::endl; }
...
std::optional<std::string> CRDTSyncEngine::join_delta_node_attr(MvregNodeAttrMsg&& mvreg)
{
    ...
    try {
        if (!protocol_version_matches(host_.log_level, "DSR_NODE_ATTS", mvreg.protocol_version)) {
            return {};
        }
        auto id = mvreg.node;
        ...
        auto d_empty = crdt_delta.empty();
        {
            ...
            if (nodes_.contains(id)) {
                ...
                return att_name;
            } else if (!host_.deleted.contains(id)) {
                bool find = false;
...
std::optional<std::string> CRDTSyncEngine::join_delta_edge_attr(MvregEdgeAttrMsg&& mvreg)
{
    ...
    try {
        if (!protocol_version_matches(host_.log_level, "DSR_EDGE_ATTS", mvreg.protocol_version)) {
            return {};
        }
        ...
        auto d_empty = crdt_delta.empty();
        {
            ...
            if (nodes_.contains(from)  and nodes_.at(from).read_reg().fano().contains({to, type}))
            {
                ...
                return att_name;
            } else if (!host_.deleted.contains(from) && !host_.deleted.contains(to)) {
                bool find = false;
...
void CRDTSyncEngine::join_full_graph(OrMap&& full_graph)
{
    ...
    const auto log_level = host_.log_level;
    if (!protocol_version_matches(host_.log_level, "GRAPH_ANSWER", full_graph.protocol_version)) {
        return;
    }
        ...
        for (auto &[signal, node_id, type, nd, current_nd] : updates) {
            if (signal) {
                if (!nd.has_value() || nd->attrs() != current_nd->attrs()) {
                    host_.emitter.update_node_signal(node_id, type, SignalInfo{ agent_id_ch });
                } else if (nd.value() != *current_nd) {
                    const auto& iter = current_nd->fano();
                    for (const auto &[k, v] : nd->fano()) {
                        if (!iter.contains(k)) {
                            host_.emitter.del_edge_signal(node_id, k.first, k.second, SignalInfo{ agent_id_ch });
                            if (v.dk.ds.size() > 0) {
                                Edge tmp_edge(v.read_reg());
                                host_.emitter.deleted_edge_signal(tmp_edge, SignalInfo{ host_.agent_id });
                            }
                        }
                    }
                    for (const auto &[k, v] : iter) {
                        if (auto it = nd->fano().find(k); it == nd->fano().end() or it->second != v)
                            host_.emitter.update_edge_signal(node_id, k.first, k.second, SignalInfo{ agent_id_ch });
                    }
                }
            } else {
                host_.emitter.del_node_signal(node_id, SignalInfo{ agent_id_ch });
                if (nd.has_value()) {
                    Node tmp_node(*nd);
                    host_.emitter.deleted_node_signal(tmp_node, SignalInfo{ agent_id_ch });
                }
            }
        }
    }
...
std::map<uint64_t, MvregNodeMsg> CRDTSyncEngine::export_mvreg_map() const
{
    ...
    for (const auto& kv : nodes_) {
        ...
        msg.dk = kv.second;
        msg.id = kv.first;
        msg.agent_id = host_.agent_id;
        msg.timestamp = get_unix_timestamp();
        msg.protocol_version = DSR_PROTOCOL_VERSION;
        msg.sync_mode = sync_mode_wire_value(host_.sync_mode);
