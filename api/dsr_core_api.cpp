...
namespace DSR
{
    ...
    Graph::Graph(CortexConfig& cfg) : config(cfg)
    {
        ...
        } else 
        {
            ...
            {
            auto [response, repeated]  = transport->start_fullgraph_request(this);    // for agents that want to request the graph for other agent
        ...
        try
        {
            ...
            std::optional<std::unordered_set<std::pair<uint64_t, std::string>, hash_tuple>> cache_map_to_edges = {};
            {
                ...
                if (!deleted.contains(id))
                {
                    joined = true;
                    maybe_deleted_node =
                        (nodes[id].empty()) ? std::nullopt : std::make_optional(nodes.at(id).read_reg());
    ...
    void Graph::join_delta_edge(IDL::MvregEdge &&mvreg)
    {
        try
        {
            ...
            auto consume_unprocessed_deltas = [&]()
            {
                ...
                std::erase_if(
                    unprocessed_delta_edge_to, [&](auto &it)
                ...
                else if (!dfrom and !dto)
                {
                    ...
                    {  // There should not be many elements in this iteration
                        if (std::get<0>(begin->second) == to && std::get<1>(begin->second) == type)
                        {
                            ...
                            std::cout << "JOIN_DELTA_EDGE ID(" << from << ", " << to << ", " << type
                                      << ") JOIN UNPROCESSED" << std::endl;
                            find = true;
                            break;
                        }
                    }
                    ...
                    if (!find)
                    {
                        ...
                        if (cfrom && !cto)
                        {
                            ...
                            unprocessed_delta_edge_to.emplace(to,
                                                              std::tuple{from, type, std::move(crdt_delta), timestamp});
                        }
                    }
                    else
                    {
...

        {
            ...
            for (auto &[k, val] : full_graph.m())
            {
                ...
                std::optional<CRDTNode> nd =
                    (nodes[k].empty()) ? std::nullopt : std::make_optional(nodes[k].read_reg());
                id = k;
    ...
    void Graph::process_delta_edge_attr(uint64_t from, uint64_t to, const std::string &type,
                                        const std::string &att_name, mvreg<CRDTAttribute> &&attr)
    {
        const bool d_empty = attr.empty();
