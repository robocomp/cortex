...
inline MvregNodeMsg CRDTNode_to_Msg(uint32_t agent_id, uint64_t id, mvreg<CRDT::Node>&& data)
{
    return crdt_node_to_msg(agent_id, id, std::move(data));
}

template<typename S>
inline MvregEdgeMsg CRDTEdge_to_Msg(uint32_t agent_id, uint64_t from, uint64_t to, S&& type, mvreg<CRDT::Edge>&& data)
{
    return crdt_edge_to_msg(agent_id, from, to, std::forward<S>(type), std::move(data));
}

template<typename S>
inline MvregNodeAttrMsg CRDTNodeAttr_to_Msg(uint32_t agent_id, uint64_t id, uint64_t node, S&& attr, mvreg<Attribute>&& data)
{
