...
inline MvregNodeMsg crdt_node_to_msg(uint32_t agent_id, uint64_t id, mvreg<CRDT::Node>&& data)
{
    MvregNodeMsg msg;
...
template<typename S>
inline MvregEdgeMsg crdt_edge_to_msg(uint32_t agent_id, uint64_t from, uint64_t to, S&& type, mvreg<CRDT::Edge>&& data)
{
    MvregEdgeMsg msg;
    msg.dk = std::move(data);
    msg.id = from;
...
template<typename S>
inline MvregNodeAttrMsg crdt_node_attr_to_msg(uint32_t agent_id, uint64_t id, uint64_t node, S&& attr, mvreg<Attribute>&& data)
{
...
inline MvregEdgeAttrMsg crdt_edge_attr_to_msg(uint32_t agent_id, uint64_t id, uint64_t from, uint64_t to,
                                              TS&& type, AS&& attr, mvreg<Attribute>&& data)
{
...
inline CRDT::Edge user_edge_to_crdt(Edge&& edge)
{
    CRDT::Edge crdt_edge;
...
inline CRDT::Edge user_edge_to_crdt(const Edge& edge)
{
    CRDT::Edge crdt_edge;
...
inline Edge to_user_edge(const CRDT::Edge& edge)
{
...
inline Node to_user_node(const CRDT::Node& node)
{
...
inline CRDT::Node user_node_to_crdt(Node&& node)
{
    CRDT::Node crdt_node;
...
inline CRDT::Node user_node_to_crdt(const Node& node)
{
    CRDT::Node crdt_node;
